...
uint8_t _rtc_is_leap_year(int year, rtc_leap_year_support_t leap_year_support)
{
    /*
    ...
    if (leap_year_support == RTC_FULL_LEAP_YEAR_SUPPORT && year == 200) {
        return 0; // 2100 is not a leap year
    }

    return (year) % 4 ? 0 : 1;
}

uint8_t _rtc_maketime(const struct tm *time, time_t *seconds, rtc_leap_year_support_t leap_year_support)
{
...
uint8_t _rtc_localtime(time_t timestamp, struct tm *time_info, rtc_leap_year_support_t leap_year_support)
{
    if (time_info == NULL) {
        return 0;
    }
