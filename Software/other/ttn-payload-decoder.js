...
function decodeUplink(input) {
    ...
    decoded.weight_kg = decoded.w_v;
    
    //vbatt
    var rawVBatt = (bytes[8] << 8) | bytes[9];
    var VBatt = rawVBatt;
    decoded.bv = VBatt;
  
    //vbatt percent
    var rawVBattPercent = ((bytes[10] & 0x80) ? (0xFFFF << 16) : 0) | (bytes[10] << 8) | bytes[11];
    var VBattPercent = rawVBattPercent / 100;
    decoded.bat_perc = VBattPercent;
  
    //scale type
    var scaleType = bytes[12];
    if(scaleType == 1) {
      delete decoded.w_v;
      delete decoded.weight_kg;
      decoded.w_br = rawWeight;
    }
    //1wire temperature
    if(bytes[13]) {
      decoded.ds18b20_present = true;
      decoded.ds18b20_sensor_amount = bytes[13];
      
      for(let i=0;i<decoded.ds18b20_sensor_amount;i++) {
        var rawTempOw = ((bytes[14+(i*2)] & 0x80) ? (0xFFFF << 16) : 0) | (bytes[14+(i*2)] << 8) | bytes[15+(i*2)];
        var temperatureOw = rawTempOw / 100;
        decoded.t_i = temperatureOw; //find correct destination key compatible with sensor amout
      }
    }
