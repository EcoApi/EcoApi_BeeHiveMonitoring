...
PACK(struct FFT_RESULTS_ {
  ...
  uint32_t values[AUDIO_MAX_BINS];
});
...
void fft_create_result(FFT_RESULTS *p_fftResult, float *p_fftValue, uint8_t binOutputCount, uint16_t binOffset, uint8_t binOutputSize, uint16_t fftSize);
