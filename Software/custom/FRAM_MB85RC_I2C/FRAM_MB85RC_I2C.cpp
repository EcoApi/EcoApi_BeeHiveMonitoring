...
boolean FRAM_MB85RC_I2C::begin(void) {
	...
	return FRAM_MB85RC_I2C::isReady();
}
...
byte FRAM_MB85RC_I2C::writeArray (uint16_t framAddr, uint16_t items, uint8_t values[])
{
...
*/
/**************************************************************************/
byte FRAM_MB85RC_I2C::readArray (uint16_t framAddr, uint16_t items, uint8_t values[])
{
	if ((framAddr > maxaddress) || ((framAddr + (uint16_t) items - 1) > maxaddress)) return ERROR_11;
